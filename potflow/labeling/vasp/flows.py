...
class RelaxMaker(Maker):
    ...
    structure_composer: StructureComposeMaker
    ...
    relax_maker_class: BaseVaspMaker = RelaxMaker
    name: str = "relax"
    ...
    def make(self, structure: Structure, prev_vasp_dir: str | Path | None = None):
        ...
        if self.relax_parent_structure:
            j = self.relax_maker_class().make(structure, prev_vasp_dir=prev_vasp_dir)
            jobs.append(j)
            j.name += "_parent"
        ...
        generated_structure: Structure | list[Structure] = composer.make(structure)

        # relax generated structures
        if not isinstance(generated_structure, [tuple, list]):
            generated_structure = [generated_structure]
        for i, struct in enumerate(generated_structure):
            j = self.relax_maker_class.make(struct, prev_vasp_dir=prev_vasp_dir)
            jobs.append(j)
